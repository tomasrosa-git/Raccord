import { Router } from 'express';
import type { RequestHandler } from 'express';
import { parsear } from '../../middlewares/validateRequest';
import { idParamsSchema } from '../persona/persona.schema';
import { colaboradoresService } from './colaboradores.service';

const obtener: RequestHandler = async (req, res, next) => {
  try {
    const { id } = parsear(idParamsSchema, req.params);
    res.json(await colaboradoresService.obtener(id));
  } catch (err) {
    next(err);
  }
};

// Se monta en /api/personas — la ruta completa es /api/personas/:id/colaboradores.
export const colaboradoresRouter = Router();
colaboradoresRouter.get('/:id/colaboradores', obtener);

...
import {
  getDueloTaquillaRonda,
  getDueloTaquillaSiguiente,
  resolverDueloTaquilla,
} from '@/lib/api/juegos';
import { EtiquetaSeccion } from '@/components/ui/EtiquetaSeccion';
import { Skeleton } from '@/components/ui/Skeleton';
import { cn } from '@/lib/utils/cn';
import { anioDe, formatearDinero } from '@/lib/utils/formatters';
import type { DueloPelicula, DueloTaquillaResultado } from '@/types';

const CLAVE_RECORD = 'raccord:duelo-taquilla:record';
 ...
 * su recaudación ya revelada como referencia; la rival la oculta hasta responder.
 */
function Carta({
  pelicula,
  onElegir,
  ...
  resultado: DueloTaquillaResultado | null;
  esGanadora: boolean;
  fueElegida: boolean;
  esCampeon: boolean;
  ...
  const recaudacion = resultado?.recaudacion[pelicula.id];
...
export function DueloTaquilla() {
  ...
  const [resultado, setResultado] = useState<DueloTaquillaResultado | null>(null);
  ...
  const nuevaPartida = useCallback(async () => {
    ...
    try {
      const ronda = await getDueloTaquillaRonda();
  ...
  async function elegir(pelicula: DueloPelicula) {
    ...
    try {
      const res = await resolverDueloTaquilla(izquierda.id, derecha.id, pelicula.id);
      setResultado(res);
  ...
  async function siguienteRonda() {
    ...
    if (!ganadora) return;
    const valor = resultado.recaudacion[ganadora.id] ?? null;
    ...
    try {
      const { rival } = await getDueloTaquillaSiguiente(ganadora.id);
      setDerecha(rival);
    } catch {
      setError(true);
    }
    ...
    <div>
      ...
      <p className="mt-6 text-center text-sm text-papel/60">
        {campeonId ? (
          ...
          <>
            ¿Cuál <span className="text-papel">recaudó más</span>?
          </>
        )}
      </p>
      ...
      {resultado && (
        <div className="mt-8 border-t border-borde pt-8">
          ...
          <p className="mt-6 font-mono text-xs text-papel/40">
            La recaudación mundial la informa TMDB, sin ajustar por inflación.
          </p>
        </div>
      )}

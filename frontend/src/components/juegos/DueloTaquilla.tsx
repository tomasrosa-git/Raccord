...
import { getDueloTaquillaRonda, resolverDueloTaquilla } from '@/lib/api/juegos';
...
import type { DueloPelicula, DueloRonda, DueloTaquillaResultado } from '@/types';
...
/** Una carta del duelo. Tras responder muestra la recaudación real. */
function Carta({
  pelicula,
  onElegir,
  deshabilitado,
...
export function DueloTaquilla() {
  ...
  const cargarRonda = useCallback(async () => {
    ...
    setElegidaId(null);
    try {
      setRonda(await getDueloTaquillaRonda());
    } catch {
      setError(true);
    } finally {
      setCargando(false);
  ...
  async function elegir(pelicula: DueloPelicula) {
    ...
    try {
      const res = await resolverDueloTaquilla(ronda.a.id, ronda.b.id, pelicula.id);
      setResultado(res);
